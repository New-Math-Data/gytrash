...
class KinesisStream(BufferedIOBase):
    """
    stream interface used by the handler which binds to Kinesis
    Firehose and uploads log records
    """
    ...
    def __init__(
        ...
        **boto_session_kwargs
        ...
        :param partition_key: Kinesis partition key
                              used to group data by shards
        :type partition_key: str
        :param chunk_size: the size of a a chunk of records
                           for rotation threshold (default 524288)
        :type chunk_size: int
        :param encoder: the encoder to be used for log records (default 'utf-8')
        :type encoder: str
        :param workers: the number of background workers that
                        rotate log records (default 1)
        :type workers: int
        :param boto_session_kwargs: additional keyword arguments
                                    for the AWS Kinesis Resource
        ...
        try:
            ...
            raise ValueError(
                "Kinesis stream %s does not exist or inactive, or insufficient permissions"  # noqa
                % stream_name
            )

        self.workers = [
            threading.Thread(
                target=task_worker, args=(self.tasks,), daemon=True
            ).start()
    ...
    def _rotate_chunk(self, run_async=True):
        ...
        :param run_async: Indicates whether the rotation should by
                          asynchronous on a different thread
        :type run_async: bool
        :return:
        """
        ...
        if buffer.tell() > MAX_CHUNK_SIZE:
            # We are limited to a size of 1 MB per stream upload command so we
            # need to enforce it
    ...
    def _upload_part(self, buffer):
        try:
            ...
            logging.exception(
                "Failed to stream to AWS Firehose data stream {}".format(
                    self.stream_name
                )
            )
...
class KinesisHandler(StreamHandler):
    ...
    def __init__(
        ...
        **boto_session_kwargs
        ...
        :type partition_key: str
        :param chunk_size: the size of a a chunk of records for rotation
                           threshold (default 524288)
        :type chunk_size: int
        :param encoder: the encoder to be used for log records (default 'utf-8')
        :type encoder: str
        :param workers: the number of background workers that rotate log records
                        (default 1)
        :type workers: int
        :param boto_session_kwargs: additional keyword arguments for the AWS
                                    Kinesis Resource
        :type boto_session_kwargs: boto3 resource keyword arguments
        """
        ...
        args_validation = (
            ValidationRule(
                stream_name, is_non_empty_string, empty_str_err("stream_name")
            ),
        ...
        # Make sure we gracefully clear the buffers and upload the missing
        # parts before exiting
