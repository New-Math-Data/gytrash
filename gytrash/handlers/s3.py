...
from gytrash.utils.tasks import STOP_SIGNAL, Task, task_worker

from gytrash.utils.validation import (  # isort:skip
    ValidationRule,
    bad_integer_err,
    empty_str_err,
    is_non_empty_string,
...
class StreamObject:
    """
    Class representation of the AWS s3 object along with all the needed metadata
    to stream to s3
    """
    ...
    def __init__(self, s3_resource, bucket_name, filename, buffer_queue, encryption):
        ...
        try:
            total_bytes = s3_resource.meta.client.head_object(
...
class S3Stream(BufferedIOBase):
    ...
    def __init__(
        ...
        *,
        ...
        max_file_log_time: int = DEFAULT_ROTATION_TIME_SECS,
        ...
        **boto_session_kwargs
        ...
        :type chunk_size: int
        :param max_file_log_time: threshold period for a log period until file
                                  rotation (default 12 Hours)
        ...
        :type encoder: str
        :param workers: the number of background workers that rotate log records
                        (default 1)
        :type workers: int
        :param compress: flag indication for archiving the content of a file
        :type compress: bool
        :param boto_session_kwargs: additional keyword arguments for the AWS Kinesis
                                    Resource
        ...
        self.workers = [
            threading.Thread(
                target=task_worker, args=(self._rotation_queue,), daemon=True
            ).start()
            for _ in range(int(max(workers, MIN_WORKERS_NUM) / 2) + 1)
        ]
        self._stream_bg_workers = [
            threading.Thread(
                target=task_worker, args=(self._stream_buffer_queue,), daemon=True
            ).start()
    ...
    def bucket(self, val):
        ...
        try:
            ...
            raise ValueError(
                "Bucket %s does not exist, or insufficient permissions" % val
            )
    ...
    def _rotate_chunk(self, run_async=True):
        ...
        if run_async:
            self._current_object.add_task(
                Task(self._upload_part, self._current_object, part, part_num, buffer)
            )
        else:
    ...
    def _close_stream(stream_object, callback=None, *args, **kwargs):
        ...
        if stream_object.chunk_count > 0:
            stream_object.uploader.complete(
                MultipartUpload={
                    "Parts": sorted(stream_object.parts, key=lambda p: p["PartNumber"])
                }
            )
        else:
    ...
    def write(self, *args, **kwargs):
        ...
        s = (
            self.compress
            and gzip.compress(args[0].encode(self.encoder))
    ...
    def flush(self, *args, **kwargs):
        ...
        if (
            ...
            and self._current_object.byte_count > self.max_file_size_bytes
        ) or (
            self.max_file_log_time
            and int(datetime.utcnow().strftime("%s")) - self.start_time
            > self.max_file_log_time
        ):
            self._rotate_file()
...
class S3Handler(StreamHandler):
    ...
    def __init__(
        ...
        *,
        chunk_size: int = DEFAULT_CHUNK_SIZE,
        time_rotation: int = DEFAULT_ROTATION_TIME_SECS,
        max_file_size_bytes: int = MAX_FILE_SIZE_BYTES,
        encoder: str = "utf-8",
        workers: int = 1,
        compress: bool = False,
        **boto_session_kwargs
        ...
        :type bucket: str
        :param chunk_size: size of a chunk in the multipart upload in bytes
                           (default 5MB)
        :type chunk_size: int
        :param time_rotation: Interval in seconds to rotate the file by
                              (default 12 hours)
        :type time_rotation: int
        :param max_file_size_bytes: maximum file size in bytes before rotation
                                    (default 100MB)
        :type max_file_size_bytes: int
        ...
        args_validation = (
            ValidationRule(
                time_rotation, is_positive_int, bad_integer_err("time_rotation")
            ),
            ValidationRule(
                max_file_size_bytes,
                is_positive_int,
                bad_integer_err("max_file_size_bytes"),
            ),
        ...
        # Make sure we gracefully clear the buffers and upload the missing parts
        # before exiting
