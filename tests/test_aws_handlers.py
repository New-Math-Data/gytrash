...
from gytrash.handlers.kinesis import KinesisHandler
from gytrash.handlers.s3 import S3Handler
...
class S3Test(Base):
    def setUp(self):
        ...
        try:
            b_objects = [
                {"Key": o["Key"]}
                for o in self.s3_client.list_objects(Bucket=self.bucket).get("Contents")
            ]

            self.s3_client.delete_objects(
                Bucket=self.bucket, Delete={"Objects": b_objects}
            )
        except Exception:
            pass
...
class KinesisTest(Base):
    ...
    def test_basic_logging_kinesis(self):
        kinesis_handler = KinesisHandler(
            ...
            chunk_size=100,
            endpoint_url=config.TEST_KINESIS_URL,
            verify=False,
        )
        ...
        shard_iterator = self.kinesis_client.get_shard_iterator(
            StreamName=self.stream_name, ShardId=shard_id, ShardIteratorType="LATEST"
        )
        ...
        try:
            while (
                "NextShardIterator" in log_records
                and log_accumulator < log_record_num * 3
            ):
                log_accumulator += sum(
                    [
                        len(d["Data"].decode("utf-8").strip().split("\n"))
                        for d in log_records["Records"]
                    ]
                )
                log_records = self.kinesis_client.get_records(
                    ShardIterator=log_records["NextShardIterator"], Limit=1
                )
        finally:
